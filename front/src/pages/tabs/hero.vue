...
<template>
	<view class="customer-services">
		...
		<div class="content-outer">
			...
			<view class="th">
				<view class="name-wrap"></view>
				<template v-for="(type) in types[selected]" :key="type.key">
					<view class="tr" v-if="!(['gj', 'dkgj', 'gs', 'hpg', 'level', 'time'].includes(type.key) && selected == 'unit')">
						<text class="td">{{ type.name }}</text>
					</view>
				</template>
			</view>

			<view class="flex ai-center fs-14" v-for="(item, index) in list.current" :key="item.id + '_' + index" :class="[index % 2 ? 'tr-wrap-0' : 'tr-wrap-1']">
				...
				<template v-for="(type) in types[selected]" :key="type.key">
					<view class="tr" v-if="!(['gj', 'dkgj', 'gs', 'hpg', 'level', 'time'].includes(type.key) && selected == 'unit')">
						<text v-if="type.key == 'hpg'" class="td">{{ getVal(item, type.key).val.toFixed(1) }}</text>
						<text v-else class="td">{{ getVal(item, type.key).val }}</text>
					</view>
				</template>
			</view>
		</div>
...
<script>
	...
	export default {
		...
		methods: {
			...
			getVal(item, type = this.type) {
				if (!item) {
					return false;
				}
				if (this.selected == "hero") {
					return this.getHeroVal(item, type);
				} else {
					return this.getUnitVal(item, type);
				}
			},
			getHeroVal(item, type = this.type) {
				if (!item.main) {
					return 0;
				}
				...
				let r,
					t,
					width = 50;
				let param = this.types.hero.find(item => item.key == type).param;
				switch (type) {
					case "gj":
						t = level[param][0];
				...
				if (type == "dps") {
					r = this.getHeroVal(item, 'gj').val * (this.getHeroVal(item, 'gs').val || 0) || 0
					r = r.toFixed(1)
					width = 130
					t = `${r}[${this.getHeroVal(item, 'gj').val}/${this.getHeroVal(item, 'gs').val}]`
				}
			...
			getUnitVal(item, type = this.type) {
				...
				if (type == "dddps") {
					var val = this.getUnitVal(item, 'gj').val * this.getUnitVal(item, 'gs').val / this.getUnitVal(item, 'rk').val
					val = val.toFixed(1)
					result = {
						val: val,
						width: 130
					};
					result.tit = `${result.val}[${this.getUnitVal(item, 'gj').val}/${this.getUnitVal(item, 'gs').val}]`
					// console.log(this.getUnitVal(item, 'gj'), this.getUnitVal(item, 'gs'), result.val)
				}
				if (type == "dkdps") {
					var val = this.getUnitVal(item, 'dkgj').val * this.getUnitVal(item, 'gs').val / this.getUnitVal(item, 'rk').val
					val = val.toFixed(1)
					result = {
						val: val,
						width: 130
					};
					result.tit = `${val}[${this.getUnitVal(item, 'dkgj').val}/${this.getUnitVal(item, 'gs').val}]`
				}
				return result;
			},
			getPercent(item) {
